...
export class FunctionsService {
  ...
  async compareExtend(QA: string, Production: string, compareBy: string, crawler: boolean, production: boolean) {
    const qaRunId = QA.split('/runs/')[1].split('#')[0];
    const prodRunId = Production.split('/runs/')[1].split('#')[0];
    const prodToken = production ? 'apify_api_94yploUc38a42oWZy6cTFvL10EKlEy20cFqn' : 'apify_api_U4Wd5n7V9USgdsBpkYDpB8WUfhkJEX3wh0LV';
    const qaToken = 'apify_api_U4Wd5n7V9USgdsBpkYDpB8WUfhkJEX3wh0LV';




    const QARun = this.apifyService.getQaRun(qaRunId);
    const datasetQAId = QARun.data.defaultDatasetId;
    const keyValQAId = QARun.data.defaultKeyValueStoreId;
    const ProdRun = this.apifyService.getProdRun(prodRunId, production);
    ...
    async function getDataQA() {
      const response = await fetch(`https://api.apify.com/v2/datasets/${datasetQAId}/items?token=${qaToken}&clean=true`, {
        agent: httpsAgent
      });
    ...
    async function getInputQA() {
      const response = await fetch(`https://api.apify.com/v2/key-value-stores/${keyValQAId}/records/INPUT?token=${qaToken}`, {
        agent: httpsAgent
      });
      const dataQA = await response.json();
      return dataQA;
    ...
    if (objetoProd.length == 0 || objetoQA.length == 0) {
      console.log('No valid resuts to compare')
      return []
    }

    const differences = await this.compare(objetoProd, objetoQA, inputProd, inputQA, crawler, compareBy)
  ...
  async compare(objetoProd: any, objetoQA: any, inputP: any, inputQ: any, crawler: any, compareBy: any) {
        ...
        for (var prop in el1.userData) {
          if (el1.userData.Manufacturer === el2.userData.Manufacturer && el1.userData.Brand === el2.userData.Brand) {
            ...
            if (el2.userData.hasOwnProperty(prop) && crawler) {
              if (el1.url != el2.url && el1.userData.Manufacturer === el2.userData.Manufacturer && el1.userData.Brand === el2.userData.Brand) {
                same = false;
              } else if (el1.userData[prop] != el2.userData[prop] && el1.userData.Manufacturer === el2.userData.Manufacturer && el1.userData.Brand === el2.userData.Brand) {
                same = false;
              }
